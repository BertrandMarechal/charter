CREATE OR REPLACE FUNCTION chr_get_chart_js_polar_area(i_chart_id INT) RETURNS json
AS $CODE$
DECLARE
                ...
                'datasets', (
                        ...
                        FROM (
                                select json_build_object(
                                        ...
                                        'backgroundColor', (SELECT array_agg(chr_get_color(0.5) FROM chrt_chart_data_chd GROUP BY chd_label_name)),
                                        'borderColor', (SELECT array_agg(chr_get_color(1) FROM chrt_chart_data_chd GROUP BY chd_label_name)),
                                        'borderWidth',1
                                ) agg
                                from chrt_chart_data_chd
                                WHERE fk_cht_chd_chart_id = i_chart_id
